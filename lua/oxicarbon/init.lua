...
local USE_256 = tonumber(vim.loop.os_getenv("TERM"):find("256"))
...
function oxicarbon.highlight(name, definition) -- {{{
...
function oxicarbon.highlight_all(groups)
  for name, _ in pairs(groups) do
    oxicarbon.highlight(name, resolve(groups, name, false))
  end
end
...
function oxicarbon.highlight_terminal(terminal_colors)
